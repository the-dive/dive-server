...
class ArrayNestedErrorType(ObjectType):
    ...
    object_errors = graphene.List(
        graphene.NonNull("utils.graphene.error_types.CustomErrorType")
    )
            ...
            if isinstance(value[0], str):
                ...
                for pos, array_item in enumerate(value):
                    ...
                    array_errors.append(
                        ArrayNestedErrorType(
                            key=key,
                            object_errors=serializer_error_to_error_types(
                                array_item, initial_data[field][pos]
                            ),
                        )
                    )
                error_types.append(
                    _CustomErrorType(
                        field=_camelize_django_str(field), array_errors=array_errors
                    )
                )
        else:
            # fallback
...
def mutation_is_not_valid(serializer) -> List[dict]:
    ...
    if not serializer.is_valid():
        errors = serializer_error_to_error_types(
            serializer.errors, serializer.initial_data
        )
