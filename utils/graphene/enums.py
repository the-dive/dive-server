...
def convert_enum_to_graphene_enum(
    enum, name=None, description=enum_description, deprecation_reason=None
):
...
def get_enum_name_from_django_field(
    ...
    if model_name is None or field_name is None:
        raise Exception(
            f"{field=} | {type(field)=}: Both {model_name=} and {field_name=} should have a value"
        )
